...
- **commit message 分两层**(规范依据 [Conventional Commits 1.0.0](https://www.conventionalcommits.org/en/v1.0.0/)):
  ...
  - **需要额外提供 commit message 的 merge**(squash、merge commit 等):**merge 时 review + 编辑 commit message** 按 Conventional Commits(不直接用 PR description 整段)。分工:PR description 给 reviewer(why、关联、CodeRabbit summary 都行);commit message 给 git 历史(精简、规范)。merge 时(网页 squash 框,或 AI 用 `gh pr merge --squash --subject --body`)把默认值(来自 desc)编辑成:`type(scope): subject` + body(开头一段 why,每行 ≤72,不列改动清单;删 review 专用的 CodeRabbit summary / `##` 标题;关联用 footer `Closes #N`)。type ∈ `feat / fix / docs / refactor / test / chore / perf / build / ci / style`,scope = 模块,subject 祈使句 ≤50、无句号,PR/issue 号不进 subject(GitHub squash 自动追加 `(#NNNN)`)。rebase 无 merge commit、无设置,靠 commit 本身规范;没描述的 PR 由 CodeRabbit summary 默认补 commit body,merge 时可不动。
