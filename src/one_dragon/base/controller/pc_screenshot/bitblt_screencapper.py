...
class BitBltScreencapper(BitBltScreencapperBase):
    ...
    内部自动管理 CAPTUREBLT 标志以捕获分层窗口，失败时自动回退到普通模式
    """
    pass
...
class BitBltFullscreenScreencapper(BitBltScreencapperBase):
    ...
    def capture(self, rect: Rect, independent: bool = False) -> Optional[MatLike]:
        ...
        with self._lock:
            ...
            screenshot = self._capture_with_retry(0, screen_width, screen_height)
            ...
            # 裁剪到窗口区域
            return self._crop_to_window(screenshot, rect, screen_width, screen_height)

    def _crop_to_window(self, fullscreen: MatLike, rect: Rect,
                        screen_width: int, screen_height: int) -> Optional[MatLike]:
        """将全屏截图裁剪到窗口区域
        ...
        Returns:
            裁剪后的截图，失败返回 None
        """
        x1 = max(0, min(rect.x1, screen_width))
        y1 = max(0, min(rect.y1, screen_height))
        x2 = max(0, min(rect.x2, screen_width))
        y2 = max(0, min(rect.y2, screen_height))
    ...
    def _capture_independent_fullscreen(self, rect: Rect) -> Optional[MatLike]:
        ...
        try:
            ...
            saveBitMap, buffer, bmpinfo_buffer = self._create_bitmap_resources(
                screen_width, screen_height, hwndDC
            )

            fullscreen = self._capture_window_to_bitmap(
                0, screen_width, screen_height, hwndDC, mfcDC,
            ...
            return self._crop_to_window(fullscreen, rect, screen_width, screen_height)
