...
class ApplicationFactory(ABC):
    ...
    @abstractmethod
    def create_config(self, instance_idx: int, group_id: str) -> Optional[ApplicationConfig]:
        """
        创建配置实例。
    ...
    def get_config(self, instance_idx: int, group_id: str) -> Optional[ApplicationConfig]:
        """
        获取配置实例。
    ...
    def get_run_record(self, instance_idx: int) -> Optional[AppRunRecord]:
        ...
        """
        key = f'{instance_idx}'
        if instance_idx not in self._run_record_cache:
