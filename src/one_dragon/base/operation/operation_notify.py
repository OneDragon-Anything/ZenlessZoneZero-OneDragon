...
def send_node_notify(
    ...
    # 确定要发送的图片
    image_to_send = operation.last_screenshot
    # 尝试获取动态属性 notification_image，如果存在则使用
    custom_image = getattr(round_result, 'notification_image', None)
    if custom_image is not None:
        image_to_send = custom_image

    # 异步推送
    operation.ctx.push_service.push_async(
        title=operation.ctx.notify_config.title,
        content=message,
        image=image_to_send if send_image else None,
    )
