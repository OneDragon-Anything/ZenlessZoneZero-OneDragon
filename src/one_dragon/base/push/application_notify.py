...
def process_node_notifications(operation: Operation, phase: str, round_result=None):
    """集中处理一个节点的所有通知。
    ...
    # 预处理：before 阶段捕获需要的截图；after 阶段读取之前的并视需要捕获截图
    if phase == 'before':
        if any(d.capture == NodeNotifyDesc.CAPTURE_BEFORE for d in notify_list):
            setattr(operation, '_node_notify_before_image', operation.last_screenshot)
        before_image = getattr(operation, '_node_notify_before_image', None)
        ...
        if any(d.capture == NodeNotifyDesc.CAPTURE_AFTER for d in notify_list):
            after_image = operation.last_screenshot
        ...
        for desc in notify_list:
            if desc.when != NodeNotifyDesc.WHEN_BEFORE:
                ...
                if desc.capture == NodeNotifyDesc.CAPTURE_AFTER:
                    ...
                    img = before_image
                send_node_notify(operation, current_node.cn, success_flag, desc, image=img, status=round_result.status)

        # 清理缓存的 before 图片
        if hasattr(operation, '_node_notify_before_image'):
            delattr(operation, '_node_notify_before_image')
