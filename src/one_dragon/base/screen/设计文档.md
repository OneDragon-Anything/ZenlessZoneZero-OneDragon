...
- **area_name**: 区域名称
- **area_type**: 区域类型，支持 `click`、`ocr`、`template`、`color`
- **pc_rect**: 区域坐标矩形
- **text**: 文本识别内容（文本区域）
...
- **color_range**: 颜色筛选范围
- **color_match_threshold**: 颜色区域命中比例阈值，仅纯颜色区域生效
...
### 9.2 识别优化建议
1. **颜色筛选**：对于特定颜色的文本，使用color_range提高识别精度；纯颜色区域可配合color_match_threshold判断命中比例
