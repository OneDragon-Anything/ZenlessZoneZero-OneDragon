...
**识别失败时的诊断日志**：`get_match_screen_name` 匹配不到任何画面时（`round_by_goto_screen` 会据此报 `未能识别当前画面`），会输出一条 WARNING 日志，列出**接近匹配**的画面（有画面标识 id_mark 但至少一个没命中）及各自未命中的标识区域。带颜色过滤（color_range）的标识区域会标注 `(颜色过滤)`，方便排查（如快捷手册靠高亮 TAB 的橙黄色文字识别，颜色或分辨率不对就会失败）：

```
未能识别当前画面，以下画面存在未命中的标识特征：快捷手册-训练(TAB-训练(颜色过滤))；快捷手册-日常(TAB-日常(颜色过滤))
```

如果没有接近匹配的画面（如候选画面没有画面标识 id_mark），也会输出一条通用的「未能识别当前画面」日志，不会静默。日志只在识别失败时输出，识别成功时不打扰正常流程。

**性能与节流**：
- **匹配仍保持短路**：画面标识匹配时，首个 id_mark 未命中即停止识别后续标识（与旧逻辑一致），只在失败诊断里记录首个未命中的标识区域，不为收集诊断额外做全量 OCR/模板匹配。
- **失败日志节流**：同一失败详情只在首次出现时输出一次；识别成功后重置节流状态，下次失败重新输出。避免导航等待循环（wait=1s）长时间未识别时每秒重复打日志刷爆日志。
...
#### 4.3.1 画面跳转操作
```python
def round_by_goto_screen(self, screen_name: str, screen_switch_debounce: float = 0) -> OperationRoundResult:
    """从当前画面跳转到目标画面"""
    # 点击切换按钮后可进入防抖期：来源画面仍在时只等待，不重复点击；
    # 来源画面消失后立即继续识别目标画面，黑屏按加载状态等待；
    # 加载等待最多持续同样的秒数，超时后恢复原有重试。
    # 默认不启用，只有容易因旧画面残留而重复点击的流程才传入秒数。
    # 1. 识别当前画面
