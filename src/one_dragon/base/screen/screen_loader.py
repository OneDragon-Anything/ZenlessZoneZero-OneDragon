...
class ScreenContext:
    ...
    def yml_file_path(self) -> str:
        return os.path.join(
            os_utils.get_path_under_work_dir('assets', 'game_data', 'screen_info'),
            '_od_merged.yml'
        )

    def reload(self, from_memory: bool = False) -> None:
        """
        重新加载配置文件
    ...
    def save(self) -> None:
        ...
        with open(self.yml_file_path, 'w', encoding='utf-8') as file:
            yaml.safe_dump(all_data, file, allow_unicode=True, default_flow_style=False)
