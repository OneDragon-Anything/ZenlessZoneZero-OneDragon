...
from zzz_od.gui.view.matrix_action.matrix_action_setting_interface import (
    MatrixActionSettingInterface,
)
...
class MatrixActionSettingDialog(PivotNavigatorDialog):

    def __init__(self, ctx: ZContext, parent: QWidget | None = None):
        super().__init__(title="矩阵行动配置", parent=parent)

        self.ctx: ZContext = ctx
        self.group_id: str = application_const.DEFAULT_GROUP_ID

    @cached_property
    def setting_interface(self) -> MatrixActionSettingInterface:
        return MatrixActionSettingInterface(self.ctx)
