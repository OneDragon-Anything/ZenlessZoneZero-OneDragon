...
if TYPE_CHECKING:
    ...
    from zzz_od.application.matrix_action.matrix_action_run_record import MatrixActionRunRecord


class MatrixActionSettingInterface(VerticalScrollInterface):

    def __init__(self, ctx: ZContext):
        super().__init__(
            content_widget=None,
            object_name="matrix_action_setting_interface",
            nav_text_cn="矩阵行动配置",
        )

        self.ctx: ZContext = ctx
        self.group_id: str = application_const.DEFAULT_GROUP_ID
        self.config: MatrixActionConfig | None = None
        self.run_record: MatrixActionRunRecord | None = None
