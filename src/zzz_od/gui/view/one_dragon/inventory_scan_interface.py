...
class InventoryScanInterface(AppRunInterface):
    ...
    def _build_special_scan_page(self) -> QWidget:
        ...
        self.scan_agent_opt = ComboBoxSettingCard(
            icon=FluentIcon.SEARCH, title="选择代理人", content="选择要扫描的特定代理人"
        )
    ...
    def _on_start_clicked(self) -> None:
        ...
        if current_mode == self.MODE_PRE_SCAN:
            # 预扫描模式：设置为更新代理人列表
            self.ctx._inventory_scan_agent_option = (
                AgentScanOptionEnum.UPDATE_AGENTS.value.value
            )
            log.info("预扫描模式：更新代理人列表")
        else:
            # 特定扫描模式：保存选择的代理人
            if hasattr(self, "scan_agent_opt") and self.scan_agent_opt:
                selected_value = self.scan_agent_opt.getValue()
                self.ctx._inventory_scan_agent_option = selected_value
    ...
    def on_report_loaded(self, report_files, translation_dict):
        ...
        dialog = QDialog(
            self, Qt.WindowType.WindowTitleHint | Qt.WindowType.WindowCloseButtonHint
        )
        ...
        self.left_card_layout.setAlignment(
        ...
        for i in range(3):
            ...
            card_content_layout.setAlignment(
                Qt.AlignmentFlag.AlignTop | Qt.AlignmentFlag.AlignLeft
            )
        ...
        for i in range(2):
            for j in range(3):
                ...
                name_text_label.setStyleSheet(
                    "font-weight: bold; font-size: 20px;"
                )  # 最大字体

                # 总得分
                total_score_label = BodyLabel()
                total_score_label.setStyleSheet(
                    "font-weight: bold; font-size: 20px;"
                )  # 最大字体
                ...
                """)
                card_layout.addWidget(
                    detail_button, 6, 0, 1, 2, Qt.AlignmentFlag.AlignCenter
                )
                # 确保按钮可见
        ...
        right_card1_layout.setAlignment(
            Qt.AlignmentFlag.AlignTop | Qt.AlignmentFlag.AlignLeft
        )
        ...
        right_card2_layout.setAlignment(
            Qt.AlignmentFlag.AlignTop | Qt.AlignmentFlag.AlignLeft
        )
        ...
        def update_disk_cards(equipped_discs, agent_name=None):
            ...
            # 转换驱动盘数据为中文
            converted_discs = processor.convert_drive_disc_stats_to_chinese(
            ...
            if agent_name:
                character_weight = processor.load_character_weight(agent_name)
            ...
            for i in range(6):
                ...
                def create_detail_clicked_handler(
                    disc_data,
                    position,
                    main_stat_key,
                    main_scr,
                    ...
                    def handler():
                        ...
                        if not disc_data:
                            ...
                            # 1-3号位主词条不参与计算
                            if 1 <= position <= 3:
                                detail_text += "主词条得分: 0.0 (不参与计算)\n\n"
                            else:
                                detail_text += "主词条得分: 0.0\n\n"

                            detail_text += "副词条:\n"
                            detail_text += "  无\n"
                            detail_text += "\n总得分: 0.0\n"
                            detail_text += "得分上限: 0.0\n"
                            detail_text += "相对得分: 0.0\n"
                            if char_weight:
                                detail_text += f"\n代理人有效权重 ({agent_name}):\n"
                                # 显示所有权重项
                            ...
                            detail_text += (
                                f"品质: {disc_data.get('rarity', '未知')}\n\n"
                            )

                            # 主词条信息
                            main_stat_key_display = disc_data.get(
                                "mainStatKeyChinese",
                                disc_data.get("mainStatKey", "未知"),
                            )
                            ...
                            if substats:
                                for substat in substats[:4]:
                                    substat_key = substat.get(
                                        "keyChinese", substat.get("key", "未知")
                                    )
                                    substat_upgrades = substat.get("upgrades", 0)
                                    # 计算该副词条的得分
                                    substat_weight = (
                                        char_weight.get(substat_key, 0)
                                        if char_weight
                                        else 0
                                    )
                ...
                if disc:
                    ...
                    # 创建并连接点击事件处理函数
                    main_stat_key = disc.get(
                        "mainStatKeyChinese", disc.get("mainStatKey", "未知")
                    )
                    detail_button.clicked.connect(
                        create_detail_clicked_handler(
                            disc,
                            int(disc_key),
                            main_stat_key,
                            main_score,
        ...
        def load_report(agent_name):
            ...
            try:
                ...
                agent_info.append(
                    f"- {gt('突破等级')}: {report.get('promotion', '未知')}"
                )
                ...
                try:
                    # 先从布局中移除旧的扫描时间标签
                    if (
                        hasattr(self, "scan_time_label")
                        and self.scan_time_label is not None
                    ):
                        try:
                            # 从布局中移除旧控件
                ...
                try:
                    if self.agent_cards:
                        ...
                        try:
                            ...
                            card1_content.setText("当前功能正在开发")
                        except Exception as e:
                        ...
                        try:
                            ...
                            skills_info = []
                            skills_info.append(
                                f"- {gt('普通攻击等级')}: {report.get('basic', '未知')}"
                            )
                            skills_info.append(
                                f"- {gt('连携技等级')}: {report.get('chain', '未知')}"
                            )
                            skills_info.append(
                                f"- {gt('特殊技等级')}: {report.get('special', '未知')}"
                            )
                            skills_info.append(
                                f"- {gt('支援技等级')}: {report.get('assist', '未知')}"
                            )
                ...
                try:
                    ...
                    # 检查控件是否有效
                    if (
                        hasattr(self, "engine_info_label")
                        and self.engine_info_label is not None
                    ):
                        if (
                            hasattr(self.engine_info_label, "isValid")
                            and self.engine_info_label.isValid()
                        ):
                    ...
                    if (
                        hasattr(self, "engine_recommend1_label")
                        and self.engine_recommend1_label is not None
                    ):
                        if (
                            hasattr(self.engine_recommend1_label, "isValid")
                            and self.engine_recommend1_label.isValid()
                        ):
                    ...
                    if (
                        hasattr(self, "engine_recommend2_label")
                        and self.engine_recommend2_label is not None
                    ):
                        if (
                            hasattr(self.engine_recommend2_label, "isValid")
                            and self.engine_recommend2_label.isValid()
                        ):
                ...
                # 更新 agent_info_label
                if (
                    hasattr(self, "agent_info_label")
                    and self.agent_info_label is not None
                ):
                    try:
    ...
    def _on_agent_weight_clicked(self) -> None:
        ...
        WEIGHT_FIELD_2_COLUMN: dict[str, int] = {
            col.display_name: idx for idx, col in enumerate(WEIGHT_COLUMNS)
        }
        ...
        def on_weight_table_cell_changed(row: int, column: int) -> None:
            ...
            if column == 1:  # 权重值列
                ...
                try:
                    ...
                    # 更新数据
                    if not isinstance(
                        self.agent_weights[current_agent][weight_type], dict
                    ):
                    ...
                    # 恢复原来的值
                    if (
                        isinstance(self.agent_weights[current_agent][weight_type], dict)
                        and "weight" in self.agent_weights[current_agent][weight_type]
                    ):
                        weight_item.setText(
                            str(
                                self.agent_weights[current_agent][weight_type]["weight"]
                            )
                        )
                    else:
        ...
        weight_table.setHorizontalHeaderLabels(
            [gt(col.display_name) for col in WEIGHT_COLUMNS]
        )
        for idx, col in enumerate(WEIGHT_COLUMNS):
            if col.width is not None:
                weight_table.setColumnWidth(idx, col.width)
            else:
                weight_table.horizontalHeader().setSectionResizeMode(
        ...
        def sync_table_size():
            # 同步宽度
            total_width = sum(
                weight_table.columnWidth(c) for c in range(weight_table.columnCount())
            )
        ...
        sync_table_size()
        weight_table.horizontalHeader().sectionResized.connect(
            lambda _, __, ___: sync_table_size()
        )
        weight_table.verticalHeader().sectionResized.connect(
            lambda _, __, ___: sync_table_size()
        )
        ...
        def on_popup_table():
            ...
            popup_weight_table.setHorizontalHeaderLabels(
                [gt(col.display_name) for col in WEIGHT_COLUMNS]
            )
            for idx, col in enumerate(WEIGHT_COLUMNS):
                if col.width is not None:
                    ...
                    popup_weight_table.horizontalHeader().setSectionResizeMode(
                        idx, QHeaderView.ResizeMode.Stretch
                    )
            ...
            def popup_sync_table_size():
                # 同步宽度
                total_width = sum(
                    popup_weight_table.columnWidth(c)
                    for c in range(popup_weight_table.columnCount())
                )
            ...
            popup_weight_table.horizontalHeader().sectionResized.connect(
                lambda _, __, ___: popup_sync_table_size()
            )
            popup_weight_table.verticalHeader().sectionResized.connect(
                lambda _, __, ___: popup_sync_table_size()
            )
                ...
                if column == 1:  # 权重值列
                    ...
                    try:
                        ...
                        if (
                            ...
                            and "weight"
                            in self.agent_weights[current_agent][weight_type]
                        ):
                            weight_item.setText(
                                str(
                                    self.agent_weights[current_agent][weight_type][
        ...
        def on_load_weights():
            ...
            update_weight_table()
            self.show_info_bar(
                gt("成功"), gt("权重数据加载完成"), icon=InfoBarIcon.SUCCESS
            )
        ...
        def on_save_weights():
            ...
            try:
                ...
                self.show_info_bar(
                    gt("成功"), gt("权重数据保存完成"), icon=InfoBarIcon.SUCCESS
                )
            except Exception as e:
                self.show_info_bar(
                    gt("失败"), f"{gt('保存失败')}: {str(e)}", icon=InfoBarIcon.ERROR
                )
        ...
        def on_reset_weights():
            ...
            if current_agent and current_agent in self.agent_weights:
                ...
                update_weight_table()
                self.show_info_bar(
                    gt("成功"), gt("权重已重置为默认值"), icon=InfoBarIcon.SUCCESS
                )
    ...
    def _load_weights_data(self) -> None:
        ...
        processor = InventoryDataProcessor()
        agent_data = processor._load_agent_data_from_yaml()

        self.agent_weights = {}
        for agent_code, agent_info in agent_data.items():
            weight = agent_info.get("weight")
            if weight is None:
                continue
            self.agent_weights[agent_code] = {}
            for stat_key, stat_weight in weight.items():
                self.agent_weights[agent_code][stat_key] = {
                    "weight": stat_weight,
                    "enabled": True,
                }
    ...
    def _on_enabled_changed(
        self, agent_name: str, weight_type: str, checked: bool
    ) -> None:
        """启用状态改变"""
        if (
            agent_name in self.agent_weights
            and weight_type in self.agent_weights[agent_name]
        ):
