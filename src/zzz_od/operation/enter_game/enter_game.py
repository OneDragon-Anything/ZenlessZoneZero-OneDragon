...
class EnterGame(ZOperation):
    def __init__(self, ctx: ZContext, switch: bool = False):
        ...
        self.use_clipboard: bool = (
            self.ctx.game_config.type_input_way == TypeInputWay.CLIPBOARD.value.value
        )  # 使用剪切板输入
    ...
    def check_screen(self) -> OperationRoundResult:
        ...
        result = self.round_by_find_area(
            self.last_screenshot, "打开游戏", "国服-账号密码进入游戏-新"
        )
    ...
    def check_login_related(self, screen: MatLike) -> OperationRoundResult | None:
        ...
        if result.is_success:
            result2 = self.round_by_find_and_click_area(
                screen, "打开游戏", "按钮-退出登录-确定"
            )
        ...
        result = self.round_by_find_and_click_area(
            screen, "打开游戏", "国服-账号密码-新"
        )
        if result.is_success:
            return self.round_success(result.status, wait=1)

        result = self.round_by_find_and_click_area(
            screen, "打开游戏", "按钮-登陆其他账号"
        )
        if result.is_success:
            return self.round_wait(result.status, wait=1)
        ...
        if result.is_success:
            return self.round_by_find_and_click_area(
                screen, "打开游戏", "B服新-同意隐私政策"
            )
        # endregion

        if (
            self.ctx.game_account_config.game_region != GameRegionEnum.CN.value.value
            and self.ctx.game_account_config.game_region
    ...
    def input_account_password(self) -> OperationRoundResult:
        if (
            self.ctx.game_account_config.account == ""
            or self.ctx.game_account_config.password == ""
        ):
        ...
        if self.use_clipboard:
            ...
            self.ctx.controller.keyboard_controller.keyboard.type(
                self.ctx.game_account_config.account
            )
        time.sleep(1.5)
        ...
        if self.use_clipboard:
            ...
            self.ctx.controller.keyboard_controller.keyboard.type(
                self.ctx.game_account_config.password
            )
        time.sleep(1.5)
        ...
        self.already_login = True
        return self.round_by_find_and_click_area(
            screen, "打开游戏", "国服-账号密码进入游戏", success_wait=5, retry_wait=1
        )
    ...
    def input_account_password_new(self) -> OperationRoundResult:
        ...
        self.already_login = True
        return self.round_by_find_and_click_area(
            screen, "打开游戏", "国服-账号密码进入游戏-新", success_wait=5, retry_wait=1
        )
    ...
    def click_drop_button(self) -> OperationRoundResult:
        ...
        return self.round_by_find_and_click_area(
            self.last_screenshot, "打开游戏", "B服新-切换账号", success_wait=0.8
        )
    ...
    def switch_bilibili_account(self) -> OperationRoundResult:
        ...
        for ocr_result, mrl in ocr_result_map.items():
            if striped_name and str_utils.find_by_lcs(
                striped_name, ocr_result, percent=0.7
            ):
                find = True
        ...
        if not find:
            masked = (
                (
                    striped_name[:1]
                    + "*" * max(len(striped_name) - 2, 1)
                    + striped_name[-1:]
                )
                if len(striped_name) >= 2
                else "*"
            )
        ...
        self.already_login = True
        return self.round_by_find_and_click_area(
            screen, "打开游戏", "B服-登录", success_wait=5, retry_wait=1
        )
    ...
    def input_account_password_intl(self) -> OperationRoundResult:
        ...
        return self.round_by_find_and_click_area(
            screen, "打开游戏", "国际服-账号密码进入游戏", success_wait=1
        )
    ...
    def check_server(self) -> OperationRoundResult:
        ...
        screen = self.screenshot()
        return self.round_by_find_and_click_area(
            screen, "打开游戏", area_name, success_wait=1
        )
    ...
    def check_screen_to_interact(self, screen: MatLike) -> OperationRoundResult | None:
        ...
        match_word, match_word_mrl = ocr_utils.match_word_list_by_priority(
            ocr_result_map, target_word_list, ignore_list=ignore_list
        )
        if (
            match_word is not None
            and match_word_mrl is not None
            and match_word_mrl.max is not None
        ):
    ...
    def match_login_error(self, screen: MatLike) -> OperationRoundResult | None:
        ...
        match_word, match_word_mrl = ocr_utils.match_word_list_by_priority(
            ocr_result_map, target_word_list
        )
        if (
            match_word is not None
            and match_word_mrl is not None
