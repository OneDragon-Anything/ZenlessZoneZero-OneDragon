...
from zzz_od.application.activity_time_check.activity_time_check_app import (
    ActivityTimeCheckApp,
)
...
class ActivityTimeCheckAppFactory(ApplicationFactory):

    def __init__(self, ctx: ZContext):
        ApplicationFactory.__init__(
            self,
            app_id=activity_time_check_const.APP_ID,
            app_name=activity_time_check_const.APP_NAME,
        )
        self.ctx: ZContext = ctx

    def create_application(self, instance_idx: int, group_id: str) -> Application:
        return ActivityTimeCheckApp(self.ctx)

    def create_config(
        self, instance_idx: int, group_id: str
    ) -> ApplicationConfig:
        return ActivityTimeCheckConfig(
            instance_idx=instance_idx,
            group_id=group_id,
        )

    def create_run_record(self, instance_idx: int) -> AppRunRecord:
        return ActivityTimeCheckRunRecord(
            instance_idx=instance_idx,
