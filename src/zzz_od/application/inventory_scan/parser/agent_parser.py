...
class AgentParser:
    ...
    def parse_ocr_result(
        self, ocr_items: list[dict[str, Any]], screenshot: MatLike | None = None
    ) -> dict | None:
        """
        解析代理人OCR结果
        ...
        try:
            ...
            for item in ocr_items:
                ...
                if position:
                    ...
                    x, y = 0, 0
                all_results.append(
                    {"text": text, "x": x, "y": y, "confidence": confidence}
                )
            ...
            matched_key = self._match_translation(agent_name, screenshot)
            if matched_key is None:
                log.warning(f"角色 {agent_name} 不在翻译表中，使用原始名称继续")
                matched_key = agent_name  # 使用原始OCR名称作为key
            else:
                agent_name = matched_key  # 使用匹配到的英文code作为agent_name
            ...
            agent_data = {
                ...
                "equippedDiscs": {},
                "equippedWengine": "",
                "scanTime": None,  # 暂时不存储扫描时间,后续在处理时添加
    ...
    def _parse_agent_name(
        self, results: list[dict], screenshot: MatLike | None = None
    ) -> str | None:
        ...
        for candidate in candidates:
            ...
            if matched_key:
                log.debug(
                    f"OCR识别: {candidate['original']} -> 修正后: {ocr_name} (置信度: {candidate['confidence']:.4f}) -> 匹配到: {matched_key}"
                )
                return matched_key

        # 如果没有匹配到，返回置信度最高的OCR结果并保存错误
        best_name = candidates[0]["name"]
        best_name_with_marker = f"{best_name}(未匹配)"
        log.warning(
            f"未在翻译表中找到匹配，使用OCR结果: {best_name} (置信度: {candidates[0]['confidence']:.4f})"
        )
        self._save_error(screenshot, f"未在翻译表中找到匹配: {best_name}", results)
        return best_name_with_marker

    def _match_translation(
        self, ocr_name: str, screenshot: MatLike | None = None
    ) -> str | None:
        """
        在翻译表中匹配角色名称（使用模糊匹配）
        ...
        Returns:
            匹配到的英文key（如"nangong_yu"），如果没有匹配则返回None
        """
        ...
        for key, char_data in character_dict.items():
            if not isinstance(char_data, dict):
                continue
            en_value = char_data.get("EN")
            if en_value == translated:
                return en_value

        log.warning(f"未找到匹配: {translated}")
        return None
    ...
    def _parse_skill_levels(self, results: list[dict]) -> dict[str, str]:
        ...
        for result in results:
            if level_60_y < result["y"] < level_7_y:
                ...
                # 只保留数字
                if text.replace("/", "").isdigit():
                    skill_numbers.append({"text": text, "x": result["x"]})
    ...
    def _save_error(
        self, screenshot: MatLike | None, error_msg: str, ocr_results: list[dict]
    ) -> None:
        """
        保存错误截图和相关信息
