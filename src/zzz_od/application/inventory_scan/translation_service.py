...
class TranslationService:
    ...
    def _load_dict(self):
        ...
        log.warning("未找到用户翻译字典，尝试从 agent/_od_merged.yml 加载角色数据")
    ...
    def _load_agent_yaml(self) -> None:
        ...
        try:
            if not os.path.exists(self.agent_yaml_path):
                log.warning(f"agent YAML 文件不存在: {self.agent_yaml_path}")
                return

            with open(self.agent_yaml_path, encoding="utf-8") as f:
            ...
            for agent in agents_data:
                ...
                self.translation_dict["character"][agent_name] = {
                    "CHS": agent_name,
                    "EN": code,
                }

            log.info(
                f"从 agent YAML 加载了 {len(self.translation_dict['character'])} 个角色翻译"
            )
    ...
    def _translate(self, category: str, name: str, target_lang: str) -> str:
        ...
        if best_match and confidence > 0.2:
            ...
            log.warning(
                f"模糊匹配失败: {name} (最佳匹配: {best_match}, 置信度: {confidence:.4f})"
            )
    ...
    def _fuzzy_match(self, name: str, category_dict: dict) -> tuple[str | None, float]:
        """
        模糊匹配，返回最佳匹配的key和置信度
