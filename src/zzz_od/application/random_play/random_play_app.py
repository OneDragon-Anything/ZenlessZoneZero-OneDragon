...
class RandomPlayApp(ZApplication):
    ...
    def choose_promoter(self) -> OperationRoundResult:
        ...
        result = self._try_match_agent(primary_agent, area)
        if result is not None:
            return result
        log.info(f'当前代理人匹配失败: {primary_agent}')

        result = self._try_match_agent(backup_agent, area)
        if result is not None:
            return result
    ...
    def _try_match_agent(self, agent_name: str, area) -> OperationRoundResult | None:
        """尝试通过名称或头像匹配代理人并点击确认。"""
        # 使用名称匹配
        result = self.round_by_ocr_and_click(
            self.last_screenshot, agent_name, area=area,
            color_range=[(230, 230, 230), (255, 255, 255)]
        )
        if result.is_success:
            time.sleep(0.5)
            return self.round_by_find_and_click_area(
                self.last_screenshot, '影像店营业', '确认', success_wait=1, retry_wait=1
            )
