...
class ZContext(OneDragonContext):
    ...
    def _clear_cached_configs(self) -> None:
        """清理缓存的懒加载配置"""
        configs_to_clear = [
            '_screenshot_helper_config', '_battle_assistant_config', '_charge_plan_config',
            '_notorious_hunt_config', '_hollow_zero_config', '_coffee_config',
            '_life_on_line_config', '_commission_assistant_config', '_random_play_config',
            '_shiyu_defense_config', '_miscellany_config', '_drive_disc_dismantle_config',
            '_notify_config', '_lost_void_config'
        ]
        for config_name in configs_to_clear:
    ...
    def _clear_cached_run_records(self) -> None:
        """清理缓存的懒加载运行记录"""
        records_to_clear = [
            '_email_run_record', '_random_play_run_record', '_scratch_card_run_record',
            '_charge_plan_run_record', '_engagement_reward_run_record', '_notorious_hunt_record',
            '_hollow_zero_record', '_coffee_record', '_city_fund_record', '_life_on_line_record',
            '_redemption_code_record', '_trigrams_collection_record', '_ridu_weekly_record',
            '_shiyu_defense_record', '_miscellany_record', '_drive_disc_dismantle_record',
            '_notify_record', '_lost_void_record', '_suibian_temple_record'
        ]
        for record_name in records_to_clear:
            if hasattr(self, record_name):
                delattr(self, record_name)
    ...
    @property
    def suibian_temple_record(self) -> 'SuibianTempleRunRecord':
        """懒加载随便观运行记录"""
        if not hasattr(self, '_suibian_temple_record'):
            from zzz_od.application.suibian_temple.suibian_temple_run_record import SuibianTempleRunRecord
            game_refresh_hour_offset = self.game_account_config.game_refresh_hour_offset
            self._suibian_temple_record = SuibianTempleRunRecord(self.current_instance_idx, game_refresh_hour_offset)
            self._suibian_temple_record.check_and_update_status()
        return self._suibian_temple_record
