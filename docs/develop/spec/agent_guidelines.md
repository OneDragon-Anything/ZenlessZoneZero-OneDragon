...
- **Fluent设计**: 前端组件优先使用 pyside6-fluent-widgets 库中现有组件。如需实现新组件，需按照 Fluent Design 实现样式效果。配置绑定通过 `ConfigAdapter` + `AdapterInitMixin` 混入实现。
...
## 配置管理
- 运行时用户配置通过 `UserConfig` / `ApplicationConfig` 管理，旧版 YAML 会在首次读取时自动迁移；环境、项目、样例等文件型配置继续使用 `YamlConfig`。
