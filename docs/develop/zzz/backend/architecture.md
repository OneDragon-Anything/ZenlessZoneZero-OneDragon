...
> `ZzzBackendContext` —— 绝区零一条龙的运行层（`ZContext`）之上的一层**传输无关**后端，把游戏感知 / 操作能力对外暴露给 MCP 与 HTTP 适配器。本文描述**当前已实现**的能力（3 个感知方法 + 运行态三件套）；未实现的扩展见 [§路线图](#路线图尚未实现)。MCP / HTTP 适配器见 [mcp.md](mcp.md) / [http.md](http.md)，进程入口见 [entry.md](entry.md)，MCP tool 设计规范见 [design-principles.md](design-principles.md)。
...
src/zzz_od/backend/
  ...
  mcp/
    __init__.py
    app.py               # create_mcp_server + 6 个 @mcp.tool + _save_screenshot
  http/
    __init__.py
    routes.py            # register_http_routes + 6 个 /game/* 处理器
  entry/
    __init__.py
...
当前已实现：上述 3 个感知方法 + 运行态三件套（`start_run` / `query_status` / `stop`，经共享 `RunSlot`）+ MCP / HTTP 适配器 + 入口 + 远程 SSH daemon（见 [remote-ssh.md](remote-ssh.md)）。后续扩展：
