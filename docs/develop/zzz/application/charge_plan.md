...
- **循环执行**：除本次已跳过的计划外，其余计划达到计划次数后进入下一轮；普通计划按计划次数扣减，已跳过的动态计划清零。
...
副本映射与单次消耗规则以[特训目标玩法说明](../../../game/gameplay/特训目标.md#电量与副本对应关系)为准。`ChargePlanApp` 读取列表分组和电量文字，按该规则生成只运行一次的临时计划。
