...
- **运行方式**：每条实战模拟室普通计划可选择“按次数运行”或“按材料数量运行”。旧配置没有该字段时仍按次数运行。
- **运行方式校验**：`run_mode` 只接受 `run_times` 和 `material_count`。配置接口拒绝其他值；旧配置文件中若出现未知值，执行时把该计划视为已完成，避免误按次数消耗电量。
- **执行前校验**：从 YAML 加载的材料计划在传送前会重新验证副本和目标材料。非法计划只在本轮跳过并记录原因，不消耗电量。
- **按材料数量运行**：从所选副本的产出材料中选择目标并填写数量。进度从启用后的战斗结算开始累计，不读取仓库已有数量；晋升、技能和音擎改装材料可选择把同系列低级材料按 3:1 逐级合成后折算。
- **循环执行**：达到一轮次数后只重置按次数运行的计划；已经达成的材料目标不会清零，避免继续消耗电量。
...
- 按次数计划用“已运行次数/计划次数”判断完成；按材料计划用“当前折算材料数/目标材料数”判断完成
...
**任务筛选流程**：
1. 检查所有计划是否完成；开启循环时只重置按次数计划，只有已达成材料目标时直接结束
2. 获取下一个未完成的计划项
3. 计算所需电量并与当前电量比较
4. 如果电量不足：
...
- 仅支持实战模拟室的基础材料、代理人晋升、代理人技能和音擎改装。自定义模板会混合多个材料系列，代理人方案培养也不提供固定材料系列，因此不开放该运行方式。
- 目标材料来自副本数据，不接受任意文字。副本数据按 A、B、C 品质从高到低保存同系列材料名，例如“特化/进阶/基础以太芯片”。
- 战斗结束后先读取通用结算页右侧的奖励格子。每个格子下方是数量，图标底色用于区分 A、B、C 品质；五张卡和增益奖励可能生成多个同品质格子，读取时全部相加。控制器截图是 RGB，品质色转换必须使用 RGB 到 HSV，不能按 OpenCV 默认的 BGR 解释。
- 奖励数量保存成功后，才根据新进度决定“再来一次”或“完成”。数量或品质无法识别时保留结算页并重试，不写入可能错误的进度。
- `material_counts` 保存每种原始材料数量。关闭合成折算时只看目标材料；开启时按 3:1 逐级折算，并保留不能整除的低级材料供后续战斗合并计算。
- 基础材料没有 3:1 合成规则，界面不显示合成折算选项。
...
```
选择预备编队 → 出战 → 加载自动战斗指令 → 等待战斗画面 → 自动战斗 → 战斗结束 → [记录材料] → 判断下一次
```

**通用战斗流程**：
1. **选择预备编队**：根据计划配置选择指定编队。点击入口后先等待原页面按钮消失，再等待黑屏结束并出现 `+ SELECT` 或队伍卡片标记，之后才识别队名；选中目标队伍后，仅把文字完整且为白色的「预备出战」视为可点击按钮，点击后返回战斗配队页
2. **出战**：点击出战按钮进入战斗
3. **加载自动战斗指令**：根据编队或计划配置加载对应的自动战斗脚本
4. **等待战斗画面**：等待战斗界面完全加载
5. **自动战斗**：执行自动战斗逻辑，监控战斗状态
6. **战斗结束**：战斗完成后更新运行次数
7. **记录材料**：材料计划从结算页奖励区域读取每个格子的数量和品质色，按副本产出表合并后保存；按次数计划跳过此步骤
8. **判断下一次**：按次数计划根据剩余次数决定是否“再来一次”；材料计划根据刚保存的材料进度决定继续或完成
...
- 配置了循环模式（`loop=True`）
- 除本次已跳过的计划外，其余计划均已完成
- 进入下一轮时，按次数计划按整轮扣减并保留超出次数，已跳过的代理人计划清零
- 材料计划的累计进度不参与循环重置；只有材料计划已完成时，本轮正常结束

**每日清零**：
- “每日重置”独立于循环模式，默认关闭
- 开启后按游戏刷新日每天首次运行前将所有计划的已运行次数清零
- 材料累计数量不会每日清零，可跨天继续完成同一个目标
