...
体力计划（ChargePlanApp）是一个自动化执行各种副本挑战的应用，支持特训目标、实战模拟室、区域巡防、专业挑战室、恶名狩猎，以及从快捷手册进入的合成电池。该应用采用基于状态机的节点图执行模式，能够智能管理电量、自动恢复电量、循环执行计划等。

## 配置项

- **循环执行**：除本次已跳过的代理人方案培养和特训目标外，其余计划达到计划次数后进入下一轮；普通计划按计划次数扣减，已跳过的动态计划清零。
...
- **特训目标**：只配置预备编队和自动战斗，不配置具体副本与计划次数。每场战斗前读取游戏内「特训目标」列表，执行第一项未完成材料；材料全部完成后执行第一项推荐驱动盘。
...
- **专业挑战室**：固定需要40电量
- **恶名狩猎**：固定需要60电量
- **特训目标**：进页前按最低 20 电量预估，避免完全无法开本时仍传送；读取列表后再按本次实际副本生成准确预估
- **合成电池**：固定需要 60 电量
...
2. 获取下一个未完成的计划项
3. 普通计划计算所需电量并与当前电量比较；特训目标先进入统一页读取本次实际项目
4. 如果电量不足：
...
- “特训目标”先进入快捷手册「训练 → 特训目标」，读取列表第一项的分组、电量和「前往」位置，再直接使用该按钮传送
- 普通计划使用快捷手册传送到指定副本位置
- 根据计划配置的标签页、分类、副本类型进行精确传送

**特训目标动态计划**：

1. 持久配置只表示“按游戏目标刷取”，不累计完成次数，界面隐藏副本类型、已运行次数和计划次数。
2. 切换到其他训练分类再切回「特训目标」，使滚动列表回到顶部。
3. 在内容区域中取纵坐标最小的「前往」，结合它上方最近的分组标题和同一行电量文字判断实际副本。首屏没有「前往」时向下查找推荐驱动盘，兼容已完成材料仍占据上方位置的页面状态。
4. 生成只运行一次的临时副本计划。该计划继承等级、预备编队和自动战斗配置，并与持久配置共用计划标识。
5. 战斗完成次数只写入临时计划，使副本流程退出“再来一次”；持久配置保持未完成。返回快捷手册后重新读取材料缺口和推荐顺序。
6. 页面没有可识别目标或当前电量不足时，将特训目标标记为本次运行已跳过，再继续检查后续计划。

| 列表信息 | 临时副本 | 本次消耗策略 |
|---|---|---|
| `20-100` | 实战模拟室 | 在页面上限和现有电量内选择最多卡片，每张 20 电量 |
| `40-80` | 专业挑战室 | 电量不少于 80 时保留燃竭模式，否则按 40 电量普通模式 |
| `60` 且分组为驱动盘 | 区域巡防 | 60 电量 |
| `60` 且不在驱动盘分组 | 恶名狩猎 | 60 电量，走深度追猎 |
...
**电量检查流程**：
```
等待入口加载 → 按计划保留或关闭燃竭模式 → 下一步 → 恢复电量/出战
```

**特殊处理**：
- 普通计划自动关闭燃竭模式，进本前按40电量预估
- 特训目标在电量不少于80时保留页面已开启的燃竭模式，并按80电量预估；否则关闭后按40电量执行
- 进入副本后点击“下一步”，如果出现恢复电量页，则返回电量不足状态
...
6. **战斗结束**：战斗完成后更新运行次数；特训目标只更新本次临时计划
7. **判断下一次**：根据剩余次数决定是否继续战斗；特训目标每次只打一场并返回快捷手册重新决策
...
- **特训目标没有可识别目标**：与代理人方案培养相同，只在本次运行中跳过，避免阻断后续计划
- **非跳过模式**：直接结束本轮计划执行
...
**循环条件**：
- 配置了循环模式（`loop=True`）
- 除本次已跳过的代理人方案培养和特训目标外，其余计划均已完成
- 进入下一轮时，普通计划按整轮扣减并保留超出次数，已跳过的动态计划清零
- 特训目标在可执行期间始终视为未完成；只有电量不足或没有目标而在本次运行中跳过后，才允许本轮结束或继续后续计划
