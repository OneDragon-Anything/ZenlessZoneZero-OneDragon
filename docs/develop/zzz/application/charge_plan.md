...
- **换队时限**：每条计划单独配置，范围为 0 到 600 秒；0 表示关闭。普通计划和双倍活动计划都可以用“S级时限”按钮填入当前副本的推荐值，也可以继续手工输入。开启后，实战模拟室、区域巡防、专业挑战室和体力模式恶名狩猎在战斗失败或超过时限时按预备编队顺序换队。
...
- “S级时限”按钮按副本填入推荐值：实战模拟室和区域巡防为 120 秒，专业挑战室为 180 秒，恶名狩猎为 300 秒。按钮只负责填入数值，不会在切换副本时覆盖已有的自定义时限。
- 换队时限从“自动战斗”节点开始计时，不包含选队、加载和暂停时间。
- 自动战斗先检查结算结果，再判断时限。当前画面已经离开战斗态时不会按配置时限退出，避免把正常结算误判成超时。
- 配置时限达到后复用已有的“战斗菜单 → 退出战斗 → 确认 → 退出失败结果”流程。原有 600 秒硬超时仍按旧的战斗超时状态结束，不触发换队。
- 功能开启时，普通战斗失败也进入换队流程。体力模式恶名狩猎会直接撤退，不再选择“倒带”重试同一队。
- 当前使用游戏内配队（`-1`）时切到预备编队 1；当前使用预备编队 N 时切到 N+1。切换后立即保存配置，退出后保留当前副本和卡片数量，复用“下一步”节点进入出战页，再重新选队并出战。该流程不会重新传送、选副本或配置卡片数量。
- 退出后无法识别“下一步”或“预备编队”入口时，按对应操作失败结束，不回退到完整副本流程。
- 双倍活动临时计划只把新的预备编队写回双倍活动配置，不保存临时的运行次数和卡片数量。
- 到达最后一个预备编队后不回绕。开启“跳过计划”时继续下一条计划，关闭时结束本轮。
- 该判断不识别结算页的实际评级。用户配置的换队时限就是“无法达到目标用时”的判断标准。
...
graph TD
    ...
    L --> T["下一步 → 选队 → 出战"]
    T --> R["战斗"]
    R -->|"失败/配置时限，有后续编队"| S["退出结果 → 写回下一队"]
    S --> T
    R -->|"没有后续编队"| Q
    R -->|"副本结束"| M{"副本执行结果"}
    M -->|成功，或未启用换队的失败| N["挑战完成"]
...
8. **顺序换队**：按每条计划的战斗时限和失败结果切换后续预备编队，不循环回绕
