...
云游戏客户端可能同时暴露多个标题相同的窗口，其中部分窗口通过截图接口只能得到黑图。`CloudGameQueue` 因此在业务层统一覆写截图入口：所有需要截图的排队节点都会先选择正确 HWND，再通过 controller 的常规截图流程获取画面，无需在“画面识别”“插队或排队”“排队轮询”等节点中分别处理。
...
没有候选、多个候选全部黑屏，或选中窗口后的常规截图失败时，`CloudGameQueue` 每秒等待并重新探测。连续失败从首次失败开始计时，达到 `10` 秒后以“未找到有效云游戏窗口”明确失败；任意一次成功截图都会清空计时。
