...
-   **职责**:
    -   负责加载和保存流水线配置文件（`.yml`）。流水线来源为**多目录**：
        - 主仓：`assets/image_analysis_pipelines/` 目录。
        - 插件：`plugins/<插件名>/assets/image_analysis_pipelines/` 目录（存在才纳入）。
...
-   **来源解析**（`load_pipeline` / `run_pipeline` / `delete_pipeline` / `rename_pipeline` 一致）：
    -   流水线名为**裸名**，不使用命名空间前缀。来源通过 `source` 参数或当前上下文区分：`''` = 主仓；插件名 = 对应插件；`None` = 按当前上下文解析。
    -   裸名加载顺序：显式 `source` → 当前插件目录（插件 Application 执行期间 `ContextVar` 自动设置）→ 主仓目录；不查其他插件。
    -   主仓流水线互相**不遮蔽**：插件应用内裸名优先自身插件，其次主仓；主仓代码裸名走主仓。
-   **当前插件上下文**：第三方插件 Application 执行期间，框架自动设置当前插件名（`cv_service.set_current_plugin`），执行结束恢复；主仓应用不设置。业务代码无需关心。
...
4.  **Step 4: 保存流水线**: 当流水线调试到最佳效果时，在顶部的输入框中为其命名，并点击“保存”或“另存为”按钮。您的流水线将被保存为一个 `.yml` 文件，存放在当前来源目录下（面板顶部「来源」下拉框可选「一条龙」或插件，流水线列表只显示所选来源的流水线，保存落在所选来源）。
5.  **Step 5: 在代码中调用**: 在您的业务代码中（例如 `TargetStateChecker`），通过 `cv_service.run_pipeline('your_pipeline_name', image)` 来调用刚刚保存的流水线，完成一次完整的CV识别。插件应用内裸名调用自动优先自身插件流水线。
...
### 4.4. 区域识别使用 cvpipe 流水线定位

区域（`screen_info` 的 `area`）可以配置 `cvpipe` 字段（值 = 流水线名，裸名，来源由当前上下文/调试器来源选择区分），让区域定位完全走流水线：

- **拿框流程**：先按区域的 `pc_rect` 裁剪 → 裁剪图跑 `cvpipe` 流水线 → 取轮廓框（`get_absolute_rects`）→ `pc_rect` 偏移 + cv 框坐标合成真实坐标。
- **候选框验证**：轮廓框逐个用区域配置的 `text`（OCR）或 `template_id`（模板）验证（二选一照旧），第一个验证通过的框立即作为结果返回。
- **寻找与点击**：寻找方法返回框；点击方法拿框后点框中心（`round_by_click_area` 对 cvpipe 区域禁止盲点，先拿框再点）。
- **画面判断**：`is_target_screen` 等画面判断走同一拿框逻辑，`id_mark` 区域配 `cvpipe` 后画面识别自动获得流水线定位能力。
- **⚠️ 使用约束**：配了 `cvpipe` 的区域**不能默认点击**——调用方可能跳过标准点击方法、自己拿框后自己点击；拿框可能为空（流水线过滤后没有坐标），调用方必须判空处理，不得默认有点可点。
- 未配置 `cvpipe` 的区域行为完全不变（现有 OCR/模板流程）。