...
- GUI 的“结束后”配置与 CLI 的收尾参数只在一条龙运行入口构造成 `AfterDoneRequest`，由一条龙 GUI/CLI 入口在 `run_application()` 返回后调用同一 finalizer。
...
通用 GUI 的 `AppRunner` 只保存本次异步调用返回的 `ApplicationRunResult`。一条龙界面在线程 `finished` 后读取该结果，再决定是否调用一条龙专属 finalizer；普通应用和 `run_app_by_item()` 不执行结束后动作。
