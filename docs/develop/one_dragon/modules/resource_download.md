...
`one_dragon.envs.update_service.UpdateService` 负责资源更新中高于下载器的编排逻辑，当前包括启动器版本检查、目标通道计算、更新包参数构造和事务替换。版本检查会先识别当前安装的是源码启动器还是集成启动器，判断、展示和下载始终使用同一种启动器及同一个目标版本。启动器先下载并解压到临时目录，确认目标文件存在后才备份旧版本并替换；失败会回滚，程序启动时也会恢复中断替换留下的备份。`OneDragonEnvContext.update_service` 提供统一实例。
