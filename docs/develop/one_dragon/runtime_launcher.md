...
集成启动器是一种将 Python 运行时直接嵌入发行包的启动方案。与源码启动器（需要用户单独安装 Python 和 uv）不同，集成启动器解压即可运行，适合首次安装或不熟悉 Python 环境的用户。
...
`LauncherDownloadCard` 提供了一个类型下拉框，让用户在「源码启动器」和「集成启动器」之间切换。切换时会：
