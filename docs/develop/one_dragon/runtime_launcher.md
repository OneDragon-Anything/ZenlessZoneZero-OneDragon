...
7. 内置 tag 无法取得或运行时不兼容 → 不 checkout，继续使用当前源码并提示更新集成启动器
8. 其他首次同步失败 → 退出；后续更新失败 → 打日志继续运行

步骤 6 的模块清理确保了主程序后续导入的是更新后的代码，而非同步过程中缓存的旧版本。首次安装包已经包含与 `.runtime` 匹配的 `src/`；正式发布包只按同版本 tag 初始化，不会退化到最新分支。`BUILTIN_TAG_UNAVAILABLE` 或 `RUNTIME_INCOMPATIBLE` 都保留当前源码继续运行，不当作 Git 仓库损坏。
