...
4. 首次运行 → 初始化仓库并 fetch；非首次 → 根据 `auto_update_code` 配置决定是否更新
5. fetch 后、checkout 前检查目标代码与当前集成运行时是否兼容
6. 同步成功 → 清除同步过程中加载的模块，调用 `importlib.invalidate_caches()`
7. 运行时不兼容 → 不 checkout，继续使用当前源码并提示更新集成启动器
8. 其他首次同步失败 → 退出；后续更新失败 → 打日志继续运行

步骤 6 的模块清理确保了主程序后续导入的是更新后的代码，而非同步过程中缓存的旧版本。首次安装包已经包含与 `.runtime` 匹配的 `src/`；首次 fetch 返回 `RUNTIME_INCOMPATIBLE` 时，保留安装包源码继续运行，而不是把模块清单问题当作 Git 仓库损坏。
