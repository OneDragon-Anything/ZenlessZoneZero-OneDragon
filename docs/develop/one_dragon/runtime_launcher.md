...
读版本号时先按字节拿到子进程输出，再自行解码，不交给 `subprocess` 的 `text=True`。子进程的输出编码由它自己的控制台代码页决定，与本进程的 locale 无关：本进程处于 UTF-8 模式（例如设置了系统环境变量 `PYTHONUTF8=1`）时，`text=True` 会按 UTF-8 解码启动器输出的 GBK 字节，解码失败让 `stdout` 变成 `None`，版本号最终读成空字符串，界面就会一直误报“未安装”并反复提示更新
