...
| 随便观 | [随便观.md](随便观.md) | suibian_temple 经营玩法;入口(interact 狮耶)+ 9 子画面**实拍 11 webp**(游历/制造坊/售卖铺/饮茶仙/邦巢/德丰大押/好物铺/自动托管/经营总览);共 7 screen_info(入口+游历/制造坊/售卖铺/饮茶仙/邦巢/德丰大押)、3 画面无(好物铺/自动托管/经营总览)待补 |
...
**已建档(move/interact/drag 类,用 `key_tap` + `drag` + `run_operation Transport` 分解,见 onboarding skill「截图获取」)**:`scratch_card`(报刊亭;刮刮卡;嗷呜对话 / 确认弹窗待补)、`trigrams_collection`(卦象集录;滑动获取卦象 / 领奖确认待补)、`random_play`(影像店营业;Transport POINT_2 + interact 进经营;营业确认弹窗待补)、`suibian_temple`(随便观;入口 interact 狮耶 + 9 子画面实拍 11 webp;3 画面无 screen_info 待补)。
