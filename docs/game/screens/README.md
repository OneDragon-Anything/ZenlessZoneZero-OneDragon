...
- `id_mark`:该区域是否用于「当前是哪个画面」的判定。框架逐个画面比对,一个画面标了
  `id_mark: true` 的区域**全部命中**才算精准命中(`is_precise`);一个都没标的画面永远不会被
  全局识别选中。例:`邮件` 画面标了 `标题-邮件` + `全部领取` 两个,两个都命中才判定为邮件画面。
- `find_area`:已经知道在哪个画面时,按「画面名 + 区域名」直接查这个区域在不在截图里,
  不参与上面的画面判定。例:`find_area(ctx, screen, '委托助手', '对话框标题')`。
