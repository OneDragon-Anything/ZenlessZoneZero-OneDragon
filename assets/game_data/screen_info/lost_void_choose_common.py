...
class LostVoidChooseCommon(ZOperation):
    ...
    def choose_artifact(self) -> OperationRoundResult:
        ...
        if result.is_success:
            ...
            # 确定失败，尝试放弃
            log.info("确定失败，尝试放弃")
            abandon_result = self.round_by_find_and_click_area(screen=screen, screen_name='迷失之地-通用选择', area_name='按钮-放弃',
                                                         success_wait=1, retry_wait=0.2)
            if abandon_result.is_success:
                return self.round_success('已放弃')
            else:
                return self.round_retry(result.status, wait=1)  # 放弃也失败，才重试
