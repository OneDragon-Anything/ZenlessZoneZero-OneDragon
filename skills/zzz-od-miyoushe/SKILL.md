...
- **① 公开活动 API(优先)**:直播间 H5 背后是 `event/miyolive/*` 系列公开接口,**匿名、仅需 `x-rpc-act_id` 头、无 DS / 无 cookie / 不触发人机**(区别于要 DS 签名的社区业务接口,见总则避风控)。三段调用即可取码,本项目 `tools/ci/update_redemption_code.py` 已是现成实现(CI 每天 21 点自动跑,落 `config/redemption_codes.sample.yml`),端点 / 参数 / 过期算法见 [design.md](design.md)。**要拉码直接复用它或照搬三段,勿开浏览器、勿重造。**
