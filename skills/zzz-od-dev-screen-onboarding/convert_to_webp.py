...
用法:
  python convert_to_webp.py <图片.png>              原地转 q90(删原 PNG)
  python convert_to_webp.py <目录>                  批量原地转 q90
...
def convert(target: Path, quality: int, out_dir: Path | None) -> None:
    ...
    for p in paths:
        ...
        assert ok, f"编码失败: {p}"
        w = p.with_suffix(".webp") if in_place else out_dir / p.with_suffix(".webp").name
        buf.tofile(str(w))
        sw = w.stat().st_size
        tot_p += sp
        tot_w += sw
        if in_place:
            p.unlink()
        print(f"{p.name}: {sp // 1024}K -> {sw // 1024}K" + ("" if in_place else f"  ({w})"))
    if len(paths) > 1:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="PNG → webp(归档代表截图 / 整屏测试 fixture)。中文路径安全。默认原地替换并删原 PNG。",
    ...
    parser.add_argument("-o", "--out", help="输出目录(保持原文件名,保留原 PNG);不指定则原地替换并删原 PNG")
