...
**transport 后角色朝向**:传送后角色朝向**继承传送前**(若传送前已在同一地图)。app(如 `scratch_card`)常假设 transport 后固定朝向来 `move_w`+`interact`;手动复现时,先传送去**别的地图**、再传送回目标地图,朝向即重置到默认,`move_w`+`interact` 才对得上。

**操作后等动画再 capture**(否则截过渡帧):底层 `click_game` / `key_tap` / `drag` **无内置等待**(等待在框架 operation round 层 `success_wait`,MCP 不经 round)。操作后画面/角色变化需 sleep 再 `capture`,否则截到旧画面(实测 F 交互后立即 capture 截大世界)。

**move 后 sleep**:move_w 后角色未到位,紧接 interact 可能失效(app 代码常 sleep 1 防 issue #2405)。手动复现 move→interact 中间 sleep ~1s。

**interact(F)长按**:interact 用 `press_time=0.2`(长按),**不是短按 tap**(短按交互无效)。

**sleep 建议值**(操作后,连续调用/capture 前):`click_game` ~1s(点 UI 画面切换)/ `key_tap` move ~1s(角色到位)/ interact ~1-2s(进场景/对话)/ esc ~0.5s / drag ~0.5s。
