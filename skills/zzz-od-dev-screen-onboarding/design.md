...
- ✅ **Transport 失败 = 地图未探索**(方法确立,2026-07-12):onboard `hou_hou_bakery`(吼吼饼铺,3.0 玩法,布亚斯特城区)时 `run_operation Transport(布亚斯特城区, 吼吼饼铺)` 两次失败 —— `get_run_status` 显「执行传送」节点、~88s 超时;server 日志看清:Transport 打开地图后 88s 一直 OCR 地图区域视图(布亚斯特城区 / 六分街 / 澄辉坪…)、选不中吼吼饼铺传送点。根因:用户**未探索布亚斯特城区**,吼吼饼铺传送点未解锁(3.0 新地图需先跑图)。→ **通用规律:传送点没出现 / Transport 选不中 = 该地图未探索**(新版本玩法 / 新城区尤甚,其他地图同理)。SKILL.md「截图获取」加排查判据;建档选 app 优先已探索解锁的。
...
- ✅ **vision 第 3 人称视角盲区**(踩坑固化,2026-07-12):suibian_temple 入口建档时 vision 判「角色背对随便观入口」,实际是**第 3 人称追尾视角** —— 摄像机在主角身后,看到主角背部 = 主角**正对前方**(狮耶/入口),vision 不懂第 3 人称语义误判「背对」,差点误导走「跨地图朝向重置」弯路(用户纠正)。→ 判据:**第 3 人称游戏,角色背部 ≠ 背对目标,是朝向前方**;vision 对朝向 / 视角的判断要谨慎,以 OCR(交互提示如「前往 XX」)+ 实际交互结果为准,别轻信 vision 的「朝向」推断。
